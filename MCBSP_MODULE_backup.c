...
#define TX_NUM 							64
#define MY_DATA_SIZE 					64
#define TX_DATA 						0x66666600
#define DMA_DES_ADDR 					0x899ff000 //dma destination addr
#define DMA_BUFFER_BLOCK_NUM 			2  //dma buffer block num
#define DATA_FLAMES_NUM 				12
//mcbsp configure
#define BYTES_PER_FLAME 				64
#define MCBSP_WORDS_PER_FLAME 			1
#define MCBSP_BYTES_PER_WORD			WORD_32BITS
#define MCBSP_FRAME_PERIOD				(MCBSP_BYTES_PER_WORD * MCBSP_WORDS_PER_FLAME)
#define MCBSP_CLKDV						200
#define MCBSP_THRSHOLD_RX				16
//dma configure
#define DMA_WORDS_PER_FRAME				MCBSP_THRSHOLD_RX
#define DMA_FRAMES_PER_BUFFER			12
//ioctrl
...
#define MCBSP_IOC_READ_DATA 			_IOR(MCBSP_IOC_MAGIC, 5, unsigned int*) //cmd of setting dma destination addr
...
void sdma_do_work(struct work_struct *data) {
	...
	dmaConfigTransfer(mcbsp_devp->chan, MCBSP_THRSHOLD_RX, 1, (MY_MCBSP+MCBSPLP_DRR_REG), 
					buf_info_p->dma_buffer_head, 1, 0, 1, 0);
...
static irqreturn_t dma_irq_handler(int irq, void *dev_id) {
	...
	unsigned int * buffer = devp->RXBUFF1;
	//close dma irq
...
//-----------------------------------------
	...
	if(count == (current_block_size/4/16)) 
	{
		//reset count
		count=0;
		//fasync_read_block_index is for avoiding current_block_index changed
		devp->fasync_read_block_index = current_block_index;
		//change dma channel destination addr
		struct dma_buffer_info *buf_info_p;
		...
		dmaConfigTransfer(devp->chan, MCBSP_THRSHOLD_RX, 1, (MY_MCBSP+MCBSPLP_DRR_REG), 
					buf_info_p->dma_buffer_head, 1, 0, 1, 0);
		put_read_buffer(devp->read_queue, &devp->current_buffer_info);
		//send async signal to app
		kill_fasync(&devp->fasync_queue,SIGIO,POLL_IN);
	...
	{
		dmaConfigTransfer(devp->chan, MCBSP_THRSHOLD_RX, 1, (MY_MCBSP+MCBSPLP_DRR_REG), 
						devp->current_buffer_info.dma_buffer_tail, 1, 0, 1, 0);
	}
	devp->sdma_block_complete = 1; //tell read process dma complete
	//clear SDMA IRQ 
	SETBIT_REGL(DMA_CSRi + devp->chan * 0x60, event);
	SETBIT_REGL(DMA_IRQSTATUS_L1, 1<<(devp->chan));
	OUT_REGL(DMA_CDACi + mcbsp_devp->chan * 0x60, 0); //clear current destination addr
...
static void dma_init(struct mcbsp_dev *devp) 
{
	...
	dmaSetCsdp(devp->chan, SRC_LITTLE_ENDIAN, SRC_ENDIAN_ADAPT, DST_LITTLE_ENDIAN, DST_ENDIAN_ADAPT, 
				 WRITE_POSTED, DST_NO_BURST, DST_UNPACKED, SRC_NO_BURST, SRC_UNPACKED, SCALAR_32BITS);
	dmaSetCcr(devp->chan, HIGH_WR_PRIORITY, EN_BUFF, TRG_SRC, NO_PREFETCH, DIS_SUPER, DIS_TRANS_COPY, DIS_CONST_FILL, 
				DST_AMODE_POST_INC, SRC_AMODE_CONST, DIS_SUSPEND, DIS_CHANNEL, HIGH_RD_PRIORITY, TRANS_FRAME, DMA_MCBSP1_DMA_RX );
	//clear CSRi
...
static irqreturn_t mcbsp_irq_handler(int irq, void *dev_id)
{
	...
	//open mcbsp irq
	OUT_REGL(MCBSPLP_IRQENABLE_REG + MY_MCBSP, IRQENABLE_XSYNCERREN | IRQENABLE_RSYNCERREN);
	return IRQ_HANDLED;
}
...
void int mcbsp_disable_irq(struct mcbsp_dev *devp, unsigned int irq)
{
	//clear irq
	SETBIT_REGL(MCBSPLP_IRQSTATUS_REG + MY_MCBSP, irq);
...
static int mcbsp_open(struct inode *inode, struct file *filp)
{
	...
	mcbsp_enable_irq(devp, IRQENABLE_XSYNCERREN | IRQENABLE_RSYNCERREN);
	printk("mcbsp open\n");
	return 0;
}
...
//------------------
	...
	//free mcbsp irq
//	mcbsp_disable_irq(devp, IRQENABLE_XSYNCERREN | IRQENABLE_RSYNCERREN);
	//remove fasync event
...
static int mcbsp_init(void)
{
	...
	//--------------------------
	printk(KERN_INFO "send data = %d", TX_NUM);
	if(mcbsp_major) {
  ...
  // Set up frames
	mcbspRxFrameSetup(MY_MCBSP, 			//use mcbsp1
					SINGLE_PHASE, 			//Rx single phase
  					...
  					MCBSP_WORDS_PER_FLAME, 	//configure Rx words per frame
  					MCBSP_BYTES_PER_WORD, 	//configure Rx bytes per word
  					MCBSP_WORDS_PER_FLAME, 	//configure Rx words per frame
  					MCBSP_BYTES_PER_WORD, 	//configure Rx bytes per word
	...
	mcbspTxFrameSetup(MY_MCBSP, 			//use mcbsp1
  					...
  					MCBSP_WORDS_PER_FLAME, 	//configure Tx words per frame
  					MCBSP_BYTES_PER_WORD, 	//configure Tx bytes per word
  					MCBSP_WORDS_PER_FLAME, 	//configure Tx words per frame
  					MCBSP_BYTES_PER_WORD);	//configure Tx bytes per word
	...
	mcbspSetTxThresh(MY_MCBSP, MCBSP_THRSHOLD_RX);
	...
	OUT_REGL(CONTROL_PADCONF_MCBSP_CLKS, 0x01000100);
